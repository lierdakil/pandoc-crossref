  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        linker-workaround =
          pkgs:
          pkgs.writeShellScript "linker-workaround" ''
            ...
            modules = [
              (
                { pkgs, lib, ... }:
                {
                  ...
                  packages.pandoc-crossref.ghcOptions = lib.optional pkgs.stdenv.hostPlatform.isMusl "-pgml=${linker-workaround pkgs}";
                }
              )
            ];
          };
        ...
        packages = {
          ...
          static =
            if system == "x86_64-linux" then
              flake.packages."x86_64-unknown-linux-musl:pandoc-crossref:exe:pandoc-crossref"
            else if system == "aarch64-linux" then
              flake.packages."aarch64-unknown-linux-musl:pandoc-crossref:exe:pandoc-crossref"
            else
              throw "Don't know ${system} target";
